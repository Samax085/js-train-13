...
function manipulateUrl(url) {
  ...
  let secUrl = newUrl.searchParams;
  ...
  secUrl.append("newParam", "newValue")
  // // Видаляємо параметр 'oldParam' з URL, якщо він існує.
  if (secUrl.has("oldParam")) {
    secUrl.delete("oldParam")
  }  
...
function searchParamsURL(url) {
  ...
  let searchUrl = newUrl.searchParams;
...
function getURLValues(url) {
  ...
  let searchParams = new Object(newUrl.searchParams)
  // Отримуємо масив ключів пошукових параметрів.
  let arrKey = Array.from(searchParams.keys()) 
